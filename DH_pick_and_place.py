...
dh_pars = [{'joint_var':'theta', 'alpha':0, 'a': 0.5, 'd': 0, 'theta': np.pi}, \
           {'joint_var':'theta', 'alpha':0, 'a': 0.5, 'd': 0, 'theta': -np.pi/2}, \
#            {'joint_var':'theta', 'alpha':0, 'a': 0.5, 'd': 0, 'theta': np.pi/7}, \
           {'joint_var':'theta', 'alpha':0, 'a': 0.5, 'd': 0, 'theta': -np.pi/2}]

manipulator = Manipulator(dh_pars)

# Specific program begins
# Ensure that required coordinates and angle requirements are realisable
pos_req = [0.5, -0.5, 0]
ang_req = [0, 0, np.pi/4]
q_req = manipulator.dh_from_req_config(pos_req, ang_req)

# trajectory = manipulator.state_A_to_B([1.25*np.pi, np.pi/4, np.pi/6], 3)
trajectory = manipulator.state_A_to_B(q_req, 1)

t_range = np.arange(0, 1, 0.001)
# q_storage = np.zeros((len(t_range), manipulator.num_joints))
# for i, tt in enumerate(t_range):
#     q_storage[i, :] = trajectory(tt)
...
ax.grid()
ax.plot(pos_req[0], pos_req[1])

line, = ax.plot([], [], 'bo-')

def init():
    ax.plot(0, 0.5, 'go-', pos_req[0], pos_req[1], 'ro-')
    line.set_data([], [])
    return line,
...
ani.save('pick-and-place.mp4', writer=writer)
...
ax1.axis([-3, 3, -3, 3])
ax1.plot(tmp_coords[:, 0, 1], tmp_coords[:, 1, 1], 'r')
ax1.plot(tmp_coords[:, 0, 2], tmp_coords[:, 1, 2], 'b')
ax1.plot(tmp_coords[:, 0, 3], tmp_coords[:, 1, 3], 'k')
plt.show()
'''